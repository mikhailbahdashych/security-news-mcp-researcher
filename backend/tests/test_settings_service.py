...
async def test_the_stored_key_is_the_only_source(db_session, monkeypatch):
    """One source, and nothing outside the database may override it.

    This used to be a three-level precedence (process environment -> ``.env`` ->
    the stored row). It is now the row alone, so an ``ANTHROPIC_API_KEY`` left in
    a shell must not silently spend on a key the Settings page never shows.
    """
...
async def test_the_stored_voyage_key_is_the_only_source(db_session, monkeypatch):
    """Same single source as the Anthropic key, and the same guard."""
    monkeypatch.setenv("VOYAGE_API_KEY", "pa-from-env-9999")

    assert await settings_service.get_effective_voyage_key(db_session) == ""
